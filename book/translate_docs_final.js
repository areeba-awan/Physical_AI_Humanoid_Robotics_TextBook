...
function translateFile(filePath) {
  ...
  // For relative links that go to other docs
  translatedBody = translatedBody.replace(/\[([^\]]+)\]\(\.\/(module-\d+-[a-z-]+)\/([^\)]+)\)/g, (match, linkText, module, path) => {
    return `[${linkText}](/ur/docs/${module}/${path})`;
  });
  
  // For relative links to quickstart
  translatedBody = translatedBody.replace(/\[([^\]]+)\]\(\.\/(quickstart)[^\)]*\)/g, (match, linkText, path) => {
    return `[${linkText}](/ur/docs/${path})`;
  });
  
  // For relative links to intro
  translatedBody = translatedBody.replace(/\[([^\]]+)\]\(\.\/(intro)[^\)]*\)/g, (match, linkText, path) => {
    return `[${linkText}](/ur/docs/${path})`;
  });
...
console.log('Starting final translation process with updated links...');
processDirectory(SOURCE_DIR, URDU_DIR);
console.log('Final translation process completed!');